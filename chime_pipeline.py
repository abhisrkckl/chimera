...
scr_cmd = f"pam -e ftscr -u {output_dir} --setnchn 64 --setnsub 1 -d {dm} {output_dir}/CHIME_{psrname}_beam_?_?????_?????.zap"
log.info(scr_cmd)
#os.system(scr_cmd)

#Post scrunching zapping. Will need to be unique per source
pzap_cmd = f'paz -z "17 36 15 7 2 0 1 5 47 40 34" -e pzap -O {output_dir} {output_dir}/CHIME_{psrname}_beam_?_?????_?????.ftscr'
log.info(pzap_cmd)
#os.system(pzap_cmd)


#Make a metafile of the fully zapped and scrunched files
metafile_cmd = f'ls {output_dir}/CHIME_{psrname}_beam_?_?????_?????.pzap > {output_dir}/{psrname}.meta'
os.system(metafile_cmd)
meta_file = "%s/%s.meta" % (output_dir,psrname)
#print(meta_file)
