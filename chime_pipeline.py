...
class PulsarConfig:
    def __init__(self, name:str, dm:float, model_portrit:str, nchan:int, nsub:int, zap_chans:str):
        self.name = name
        self.dm = dm
...
class Session:
    ...
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(description="Generate TOAs from fold mode CHIME data.")
        parser.add_argument("-i", "--input_dir", required=True, help="Directory where input archives are stored.")
        parser.add_argument("-o", "--ouput_dir", required=True, help="Directory where output files will be stored.")
    ...
    def process_config(self):
        with open(self.config_file, 'r') as cf:
            ...
            try:
                ...
                raise AttributeError("The JSON config is malformed or missing attributes.")
...
if __name__ == "__main__":
    ...
    for pulsar in session.pulsars:
        ...
        input_ar_files = glob.glob(f"{session.input_dir}/{pulsar.datafile_glob_prefix}.ar")
